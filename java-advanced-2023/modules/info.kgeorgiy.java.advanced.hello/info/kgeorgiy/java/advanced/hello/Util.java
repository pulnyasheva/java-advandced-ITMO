...
public final class Util {
    ...
    static enum Mode {
        ...
        I18N((request, random) -> {
            return ((Random) random).nextInt(3) == 0 ? NORMAL.response((String) request, (Random) random) : Util.i18n((String) request, (NumberFormat)Util.select(Util.FORMATS,(Random) random));
        }, Util.CORRUPTIONS),
        EVIL((request, random) -> {
            return I18N.response((String)((BiFunction)Util.select(Util.EVIL_MODIFICATIONS, (Random) random)).apply(request, random), (Random) random);
